...
bool XML::SaveXmlL(QString key, QString file_name, vector<string> parameter, vector<string> value)
{
	if (key != "59226f4700f1cc8a5de755d501af308d2d2")
	{
		return false;
	...
	try
	{
		...
		{
			QDomElement param = doc.createElement(QString::fromStdString(parameter[i])); //创建子元素
			QDomText text; //设置括号标签中间的值
			text = doc.createTextNode(QString::fromStdString(value[i]));
			book.appendChild(param);
			param.appendChild(text);
		}
