...
void VideoBox::InitMenus(int iCameraCount, QMenu *menu, const QList<bool> &enable)
{
	types.clear();
	InitCamera(iCameraCount, 2, types);
	InitCamera(iCameraCount, 3, types);
	//按照行列数生成的可以自行添加其他布局
	types.insert("x",	QStringList() << "1_4x1" << "1_2x4" << "9_2x4" << "1_4x2" << "9_4x2" << "1_2x5" << "10_2x5" << "1_5x2" << "1_6x2" << "1_7x2");

	InitCamera(iCameraCount, 4, types);
	InitCamera(iCameraCount, 6, types);
	InitCamera(iCameraCount, 8, types);
	InitCamera(iCameraCount, 9, types);
	InitCamera(iCameraCount, 13, types);
	InitCamera(iCameraCount, 16, types);
	InitCamera(iCameraCount, 25, types);
	InitCamera(iCameraCount, 36, types);
	InitCamera(iCameraCount, 64, types);
	initMenu(menu, enable);
}

void VideoBox::InitCamera(int iCameraCount, int Index,QMap<QString, QStringList>& types)
{
	int	_iIndex			= iCameraCount / Index;
	int	_iMod			= iCameraCount % Index;
	int	_iStart			= 1;
	int	_iEnd			= Index;
	QStringList			_Lst;
	if (_iMod != 0)		_iIndex = _iIndex + 1;
	for (int i = 0; i < _iIndex; i++)	{
		_iStart			= 1 + Index * i;
		_iEnd			= Index + Index * i;
		QString _str	= QString("%1_%2").arg(_iStart).arg(_iEnd);
		_Lst.push_back(_str);
	}
	types.insert(QString::number(Index), _Lst);
}
...
void VideoBox::change_video_8(const QList<int> &indexs)
{
	...
	if (widgets.count() > indexs.at(6))
    gridLayout->addWidget(widgets.at(indexs.at(6)), 3, 1, 1, 1);
	if (widgets.count() > indexs.at(7))
