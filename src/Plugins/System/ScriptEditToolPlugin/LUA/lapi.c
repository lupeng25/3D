...
LUA_API void lua_recallk(lua_State *L,int nargs,int nresults,lua_KContext ctx,lua_KFunction k) 
{
	...
	if (k != NULL && yieldable(L)) {  /* need to prepare continuation? */
		L->ci->u.c.k = k;  /* save continuation */
		L->ci->u.c.ctx = ctx;  /* save context */
		luaD_recall(L, func, nresults);  /* do the call */
	}
	else  /* no continuation or no yieldable */
		luaD_recallnoyield(L, func, nresults);  /* just do the call */
	adjustresults(L, nresults);
	lua_unlock(L);
}
...
static void f_prcall(lua_State *L, void *ud) {
	struct CallS *c = cast(struct CallS *, ud);
	luaD_prcallnoyield(L, c->func, c->nresults);
}
...
LUA_API int lua_prcallk(lua_State *L, int nargs, int nresults, int errfunc,
	...
	{
		c.nresults = nresults;  /* do a 'conventional' protected call */
		status = luaD_pcall(L, f_prcall, &c, savestack(L, c.func), func);
	}
	else		/* prepare continuation (call is already protected by 'resume') */
	{
		CallInfo *ci = L->ci;
