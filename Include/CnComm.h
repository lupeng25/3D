...
#define ON_COM_PERR				ON_COM_MSG_BASE + 13//!< EV_PERR
    ...
    #define CN_ASSERT(E)   ((E) ? true : false)
#endif
...
#ifndef CnComm_EXPORTS
#define CnComm_API __declspec(dllexport)
#else
#define CnComm_API __declspec(dllimport)
#endif

class CnComm;
class ComRecieveData
{
public:
	virtual	int	Recieve(CnComm * con,QString& str) = 0;
	virtual	int	ComDisConnect() = 0;
};

class CnComm_API CnComm
{
private:
	CRITICAL_SECTION	RecieveCS_;				//!< 临界互斥锁
public:
	std::vector<ComRecieveData*> m_vecPtrRecieveData;
	void RegisterRecieveData(ComRecieveData* ptr) {
		::EnterCriticalSection(&RecieveCS_);
		for (int i = 0; i < m_vecPtrRecieveData.size(); i++) {
			if (m_vecPtrRecieveData[i] == ptr) {
				::LeaveCriticalSection(&RecieveCS_);
				return;
			}
		}
	...
	void RemoveRecieveData(ComRecieveData* ptr) {
		::EnterCriticalSection(&RecieveCS_);
	...
	//void RegisterData(ComRecieveData* ptr) { m_PtrData = ptr; };
	virtual	int	Recieve(CnComm* socket, QString& str) { return 0; };
...
public:
	...
	void RstReadEvent() {	if (hReadEvent_ != nullptr)	ResetEvent(hReadEvent_);	}
	DWORD ReadWaitString(LPVOID pBuffer, DWORD dwLength, DWORD dwWaitTime = INFINITE) {
		WaitForSingleObject(hReadEvent_, dwWaitTime);
		RstReadEvent();
		return ReadPort(pBuffer, dwLength);
	}
	int WaitMsg(QString&strMsg, int dwWaitTime = INFINITE) {
		int _iRetn = WaitForSingleObject(hReadEvent_, dwWaitTime);
		RstReadEvent();
		if (_iRetn == WAIT_TIMEOUT)	{
			return -1;
		}
	...
	//! 读取串口 dwLength - 1 个ANSI字符到 szBuffer 返回 C   适合一般字符通讯
...
protected:
	...
	virtual void Init() 
	{
		...
		::InitializeCriticalSection(&RecieveCS_);
		...
		RO_.hEvent = ::CreateEventA(NULL, TRUE, FALSE, NULL);
		CN_ASSERT(RO_.hEvent != NULL); 
		
		WO_.hEvent = ::CreateEventA(NULL, TRUE, FALSE, NULL);
		CN_ASSERT(WO_.hEvent != NULL);
		
		EO_.hEvent= ::CreateEventA(NULL, TRUE, FALSE, NULL);
		CN_ASSERT(EO_.hEvent != NULL); 
	#endif

		hWatchEvent_	= ::CreateEventA(NULL, TRUE, FALSE, NULL);
		CN_ASSERT(hWatchEvent_ != NULL); 
		hReadEvent_		= ::CreateEventA(NULL, TRUE, FALSE, NULL);
		CN_ASSERT(hReadEvent_ != NULL); 
		hWriteEvent_	= ::CreateEventA(NULL, TRUE, FALSE, NULL);
	...
	virtual void Destroy()
	{
		...
		::DeleteCriticalSection(&RecieveCS_);
	}
	//! 绑定串口 
	...
	virtual void OnReceive() 
	{
		...
		char _strArray[1024];
		memset(_strArray, 0, 1024);
		ReadPort(_strArray, 1024);
		QString _str = QString(_strArray);
		Recieve(this, _str);
		m_strRecieveMsg = _str;

		::EnterCriticalSection(&RecieveCS_);
		for (auto iter : m_vecPtrRecieveData)	iter->Recieve(this, _str);
		::LeaveCriticalSection(&RecieveCS_);
		if (hReadEvent_ != nullptr)	SetEvent(hReadEvent_);
