...
const int S_Error		= 0b1001;
const int S_Alarm		= 0b1010;
const int S_Complete	= 0b1011;
const int S_Alter		= 0b1100;

const int S_BREAK = 0b001;
const int S_LABEL = 0b010;
const int S_ARROW = 0b100;
const int S_LAARROW = 0b1000;
const int SymbolMarkers[] =
{
	{ S_BREAK },
	{ S_LABEL },
	{ S_ARROW },
...
public:
	...
	inline bool isBreak(unsigned mode) { return ((mode & S_BREAK) == S_BREAK); }
	inline bool isLabel(unsigned mode) { return ((mode & S_LABEL) == S_LABEL); }
	inline bool isArrow(unsigned mode) { return ((mode & S_ARROW) == S_ARROW); }
	inline bool isLAArrow(unsigned mode) { return ((mode & S_LAARROW) == S_LAARROW); }
	inline bool isMarker(unsigned mode,int markerNumber) { return ((mode & SymbolMarkers[markerNumber]) == SymbolMarkers[markerNumber]); }
    ...
    int markerAddSignal(int linenr, int markerNumber);
	...
	//!
	//! \sa markerAdd()
	void GotoLineSignal(int line);

	//! Returns the 32 bit mask of marker numbers at line number \a linenr.
	//!
	//! \sa getMarkers()
	void GetAllMarkers(int markerNumber,QVector<int>& lines);
    ...
    void markerDeleteSignal(int linenr, int markerNumber = -1);
    ...
    void markerDeleteAllSignal(int markerNumber = -1);
	...
	void uncomment();
public slots:
	virtual	void GotoContentLine(int iLine);

	virtual	void markerContentDelete(int linenr, int markerNumber);

	virtual	int markerContentAdd(int iLine, int markerNumber);

	virtual	void markerContentDeleteAll(int markerNumber);

	//! Writes the current document to the \a io device and returns true if
	//! there was no error.
	//!
	//! \sa read()
	virtual void updateSymbol(int, int, Qt::KeyboardModifiers);
	...
	//!
	//! \sa lexer()
	virtual void addLexerString(int iKEYWORD,QString str);
	...
	//! Get Zooms Scale the text by making the base font size \a size points and
	virtual int getzoom();
signals:
	void SendGotoLine(int iline);

	void SendmarkerContentDelete(int linenr, int markerNumber);

	int SendmarkerContentAdd(int iLine, int markerNumber);

	void SendmarkerContentDeleteAll(int markerNumber);
...
public:
	...
	int AddStyledText(QString str,int iPos,int istyle);
	int SetStyledText(int iPos,int iLenth,int istyle);
private:
    ...
    bool getSeparator(int &pos,QString& separator) const;
